...
class AdjacencyMatrixUndirectedGraphTest {
    ...
    void testRemoveNodeValid() {
        AdjacencyMatrixUndirectedGraph<String> graph = new AdjacencyMatrixUndirectedGraph<>();
    ...
    void testMatrixRemainsSquareAfterNodeRemoval() {
        ...
        // Controlla che la matrice sia ancora quadrata
        int size = graph.matrix.size();
        for (ArrayList<GraphEdge<String>> row : graph.matrix) {
            assertEquals(size, row.size(), "La matrice non è quadrata dopo la rimozione del nodo");
        }
    }
